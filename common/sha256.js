...
(() => {
	...
	function combineList (hashes) {
		var result = [], total = hashes.length;
		for (let i = 0; i < 32; i ++) {
			let list = [];
			for (let j = 0; j < total; j ++) {
				let item = hashes[j];
				let c = item[1][i];
				list[c] = (list[c] || 0) + item[0];
			}
			let max = 0, maxes = [];
			list.forEach((c, i) => {
			...
			maxes = Math.round(maxes.reduce((a, b) => a + b, 0) / maxes.length);
			result[i] = maxes;
		}
		return result;
	}
	...
	window.SHA256.toHexArray = s => {
		s = Utf8Encode(s);
		s = binb2hex(core_sha256(str2binb(s), s.length * chrsz));
		s = s.split('').map(s => parseInt(s, 16));
		var result = [], r = false, x;
		s.forEach(s => {
			if (r) {
				x = x * 16 + s;
				result.push(x);
	...
	window.SHA256.FingerPrint = article => {
		article = article.replace(/[,\.;:\?!'"\-\+=\(\)\[\]\{\}\\\/@#\$\^%&\|\*_<>`~，。？！《》；：‘’“”【】（）·￥…—　\t]+/g, ' ').replace(/ +/g, ' ');
